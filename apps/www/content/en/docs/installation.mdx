...
<Callout type="info" title="Recommendation">
  If you're starting from scratch, set up aliases (`@/*`) and Tailwind first. That avoids
  errors when running `liminal init` for the first time.
</Callout>
...
<Steps>
  <Step title="TypeScript">
    Whether your project uses TypeScript so components are generated with types.
  </Step>
  <Step title="React Server Components">
    Whether you use App Router or an RSC setup so the CLI adds `"use client"` when needed.
  </Step>
  <Step title="Components path">
    Output path for your UI components, e.g. `src/components/ui`.
  </Step>
  <Step title="Utils path">
    Path for shared utilities, e.g. `src/lib`.
  </Step>
</Steps>
